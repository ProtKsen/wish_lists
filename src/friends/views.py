...
def no_pending_requests_required(func):
    def wrapper(request, *args, **kwargs):
        ...
        pending_request_from_user = FriendRequest.objects.filter(
            from_user=request.user, to_user=friend, status=FriendRequest.RequestStatus.PENDING
        )
        pending_request_to_user = FriendRequest.objects.filter(
            from_user=friend, to_user=request.user, status=FriendRequest.RequestStatus.PENDING
        )
...
def all_friends(request):
    all_friends_objects = Friend.objects.filter(user=request.user)
    context = {"friends": [object.friend for object in all_friends_objects]}
    ...
    for object in all_users_objects:
        ...
        if not is_friend:
            request_from_user = FriendRequest.objects.filter(
                from_user=request.user, to_user=object, status=FriendRequest.RequestStatus.PENDING
            )
            request_to_user = FriendRequest.objects.filter(
                from_user=object, to_user=request.user, status=FriendRequest.RequestStatus.PENDING
            )
        ...
        users.append(
            {"id": object.id, "name": object.username, "is_friend": is_friend, "status": status}
        )
...
def all_friend_requests(request):
    pending_requests = FriendRequest.objects.filter(
        to_user=request.user, status=FriendRequest.RequestStatus.PENDING
    )
...
def accept_request(request, id: int):
    ...
    friend_request = FriendRequest.objects.get(
        to_user=request.user, from_user=friend, status=FriendRequest.RequestStatus.PENDING
    )
